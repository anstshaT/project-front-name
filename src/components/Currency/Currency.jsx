...
const Currency = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://api.monobank.ua/bank/currency')
      .then(res => {
        const dataArray = Array.isArray(res.data) ? res.data : res.data.rates || res.data.data || [];
  ...
  return (
    <div className={s.exchange}>
      ...
      {rates.map(rate => (
        <div className={s.currencies} key={`${rate.currencyCodeA}-${rate.currencyCodeB}`}>
          <span className={s.titleCurrency}>{getCurrencyLabel(rate.currencyCodeA)}</span>
          <span className={s.titleCurrency}>{formatRate(rate, 'rateBuy')}</span>
          <span className={s.titleCurrency}>{formatRate(rate, 'rateSell')}</span>
        </div>
      ))}
