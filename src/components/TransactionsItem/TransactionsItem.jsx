     ...
     <>
        <li className={`${css.transactionItem} ${
           ...
           <div className={css.field}>
                ...
                <span className={`${css.value} ${type === 'income' ? css.income : css.expense}`}>{sum.toFixed(2)}</span>
          </div>
          <div className={css.actions}>
       
          <button className={css.deleteBtn} onClick={() => dispatch(deleteTransaction(id))}>Delete</button>
       
          <button className={css.editBtn} onClick={() =>setIsModalOpen(true)}>
          <MdOutlineEdit style={{ marginRight: '4px' }}/><span className={css.editText}>Edit</span></button>
       </div>
        </li>
